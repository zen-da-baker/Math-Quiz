...
async function networkRequest(event) {
    console.log(event);
    let { request } = event;
    let cacheStorage = self.caches;
    let cache = await cacheStorage.open(cacheName);
    let matchObject = await cache.match(request);
    console.log(matchObject);
    if (matchObject) {
        event.respondWith(matchObject);
    }
    else {
        event.respondWith(fetch(event.request).catch((reason) => {
            console.log(reason);
        }));
    }
}
...
self.addEventListener("fetch", (event) => {
    console.log("Fetching a resource");
    console.log(event);
});
